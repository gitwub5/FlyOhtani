# FlyOhtani 구현 작업 안내

목표는 실제 초파리 크기의 몸과 실제 연결망에서 유도한 회로로, 눈에 보이는
공을 배트로 쳐내는 것을 학습시키고, 그 과정(회로 구조·시각 입력·스파이크)을
시뮬레이션으로 관찰하는 것이다.

## 읽는 순서

필독은 이 파일 + 아래 2개뿐이다:

1. `docs/PLAN.md` — 결정의 단일 기준. 설계 결정(D01~D36), Phase와 게이트
2. `docs/records/STATUS.md` — 지금 성립하는 값, 게이트, 알려진 한계, 다음 작업

그 다음, **맡은 작업에 해당할 때만** 읽는다:

- 뇌·정책 작업 → `docs/records/BRAIN-CIRCUIT.md`
  (망막→LIF→트리거, 그리고 셔플 대조군이 통과한 사실과 그 진단)
- 학습·보상 작업 → `docs/records/LEARNING-01.md`
  (무엇을 학습하고 무엇을 기준선으로 삼는가, 희소 보상)
- 구종·구속을 건드릴 때 → `flyshohei/pitch.py`의 `ARSENAL` 주석
  (무엇이 지금 되고, 휘는 공에 무엇이 더 필요한지 적혀 있다)
- 눈·투구·커리큘럼 작업 → `docs/records/VM-01-EYE-RATE.md`
  (보이는가에 대한 세 번의 측정과 거기서 나온 설계 법칙)
- 몸·접촉 작업 → `docs/records/G1-FORELEG-SWING.md`
  (앞다리 실측, 잠근 DOF와 그 이유, `boundmass` 함정)
- 타석 장면(몸 자세·배트·눈·충돌 설정) → `docs/records/BATTER-SCENE.md`
  (머리말에 D31 이후 바뀐 값이 정리돼 있다)
- 접촉·dt 작업 → `docs/records/G2-CONTACT.md` (v1 실패와 v2 통과의 무게 차이)
- 속도·힘 수치를 쓸 때 → `docs/records/LIT-01-FLY-LEG-LIMITS.md`
  (무엇이 측정이고 무엇이 우리가 정한 값인지 구분돼 있다)
- v1에서 넘어온 사실 → `docs/records/PRIOR-FINDINGS.md`
- 외부 데이터/코드 반입 → `docs/RESEARCH_SOURCES.md`

## 코드 지도

최상위 패키지가 둘이다. `flyshohei/`가 던지고, `flyohtani/`가 친다.

| 패키지 | 무엇 |
| --- | --- |
| **`flyshohei/`** | **던지는 쪽.** `pitch`(구종·구속·릴리스 기하), `body`(마운드 위 투수 파리) |
| `flyohtani/world/` | **치는 쪽.** `batter`(장면), `park`(구장 장식), `swing`(스윙·타구 물리), `rollout`(빠른 에피소드), `poses`(자세 탐색), `contact` |
| `flyohtani/brain/` | `connectome`(배선), `retina`(인코딩), `circuit`(LIF), `replay`(뷰어) |
| `flyohtani/task/` | `env`, `observation`(정책 계약), `outcome`(정답), `rewards`, `pitches`, `policy`, `learn` |
| `flyohtani/record/` | 영상·사진. 정책을 넘기면 정책이 스윙 시점을 정한다 |
| `flyohtani/studies/` | 일회성 측정(G1·G2·batter_check·VM-01). **라이브러리가 쓰지 않는다** |
| `flyohtani/sense/` | v1의 고전 CV 파이프라인. 지금 경로는 쓰지 않는다(금지 필드 목록만 재사용) |

`flyshohei/pitch.py`는 `flyohtani.units` 말고 아무것도 임포트하지 않는다(leaf).
그래서 `flyohtani/world/batter.py`가 그것을 임포트한다 — **반대 방향으로 쓰면
순환이 된다.** 투수 파리의 몸(`flyshohei/body.py`)만 방향이 반대라(타자의 몸
복사 기계를 쓴다) 장면 조립 중에 늦게 임포트한다.

**스윙·접촉·타구 계산은 `flyohtani/world/swing.py` 하나뿐이다.** 예전에 세 곳에 복사돼
있었고 실제로 어긋나 버그가 났다 — 새 경로를 만들지 말고 그것을 부른다.

문서 목록은 `docs/README.md`가 갖는다. 늘리기 전에 기존 문서에 들어갈 자리가
없는지 본다.

## 역할과 작업 원칙

- **사용자와 Claude가 단독으로 진행한다**(D25). 계획·설계·검토·구현·시험이
  전부 Claude에게 있다. 독립 검토자가 없으므로 `docs/PLAN.md`의
  "단독 진행의 보완 장치" 5개를 지킨다 — 특히 **수용 기준을 실행 전에
  고정**하고, **한 숫자를 믿기 전에 흔들어 본다**(dt 수렴·교차검증·민감도).
- 사용자가 지정한 작업 단위를 구현한다. 독립적인 기반 작업은 미결정 연구
  항목 때문에 멈추지 않는다.
- 설계 결정을 스스로 내리게 됐으므로, 기록에서 **"이렇게 정했다(공학적
  선택)"와 "이렇게 나왔다(측정)"를 섞지 않는다.**
- **미결정 연구 선택을 임의의 수치로 채우지 않는다.** 의존하는 작업만
  대기하고 독립 작업을 계속한다. 값을 정해야 한다면 그것이 공학적 선택임을
  명시하고 근거를 남긴다.
- 설치 성공, 물리 검증, 학습 성능을 가정하지 않는다. 실행해서 확인한다.
- **테스트 통과와 연구 가설의 검증을 구분한다.** 실행 결과는
  `docs/records/VALIDATION_LOG.md`, 상태 변화는 `docs/records/STATUS.md`.
- 실패·무효 결과를 보존한다. 결과가 나쁘다고 시드를 제외하거나 과거 실행을
  삭제하지 않는다.
- 연구 설계 변경은 `docs/PLAN.md`에 반영한다. 날짜·변경 전후·이유·이미
  열람한 결과 범위를 남긴다.
- 외부 코드·데이터의 출처·버전·체크섬·라이선스를
  `docs/RESEARCH_SOURCES.md`와 데이터 manifest에 기록한다.
- 실험과 무관한 폴더 이동·전체 재작성보다 책임 분리와 필요한 변경을
  우선한다.

## viewer/ 라이선스 조건 (D30)

`viewer/`는 fly-connectome-template(Cobanov Template Attribution License 1.0)이다.
**화면의 제작자 표시(`Attribution.tsx`)와 루트 README의 표시를 지우지 않는다.**
`viewer/` 안을 수정하면 `viewer/MODIFICATIONS.md`에 적는다.

## 게이트

Phase는 게이트를 통과해야 다음으로 넘어간다. 기준은 **실행 전에** 고정하고,
결과가 나쁘다고 완화하지 않는다.

| 게이트 | 질문 | 상태 |
| --- | --- | --- |
| G1 | 파리 앞다리가 배트를 실제로 휘두를 수 있는가 | **PASS(조건부)** — 속도 수치는 과제 설계에 쓸 수 없다 |
| G2 | 접촉이 사전 등록 수용치를 통과하는가 | v1 **FAIL** → v2 **PASS(탐색)**. 타석 장면은 D29의 짧은 확인(K1~K6) **PASS**로 대체 |
| G3 | 학습을 시작해도 되는 상태인가 (누출·exploit·예산) | **미실행** — 학습은 이미 돌았으므로 **이 게이트가 밀렸다는 사실 자체를 기록**한다 |
| G4 | MaleCNS 루밍 서브그래프를 실제로 받을 수 있는가 | **PASS** |
| VM-01 | 파리가 투구를 볼 수 있는가 | v1·v2 FAIL → 공 2배 + 조준한 20° 눈으로 성립 |

**충돌 설정이나 공·배트 치수를 바꾸면 `python -m flyohtani.studies.batter_check`를
다시 돌린다.** 스윙 자세를 바꾸면 접촉까지 걸리는 시간(`SWING_TO_CONTACT_S`)이
움직이고, 그러면 비행 시간과 필요한 눈 프레임률이 함께 움직인다 — 회귀 테스트가
잡지만, 왜 움직였는지는 직접 적어야 한다.

## 다음 작업

`docs/records/STATUS.md`의 "다음 작업" 절을 따른다. 완료된 작업의 지시를
이 파일에 쌓지 않는다 — v1에서 이미 끝난 일이 최신 작업처럼 보이는 문제를
만들었다.

## 이전 버전

v1(사람 크기 구장 + 3축 강체 배트 + 인간식 스윙 협응 탐색) 전체는 git 태그
`archive/human-scale-v0`에 있다. 그 접근으로 돌아가지 않는다.

이 파일은 인계 안내이며 현재 사용자 지시가 우선한다.

"""VM-01 V2 (user follow-up to VM-01's V1): adds TEMPORAL tracking and
PREDICTION on top of vision/ball_detector.py's per-frame classical
detector -- vision/tracker.py's BallTracker only ever held the last raw
detection static and grew a heuristic uncertainty with age; it never
...
Like vision/tracker.py, this module has NO import of vision.evaluator or
envs.* -- checked by the same AST-based test as vision/tracker.py
(tests/test_vision_v1.py, extended for this module in
tests/test_vision_v2.py). Ground-truth state is used ONLY by the

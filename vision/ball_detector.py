"""VM-01 V1: classical (non-learned) single-frame ball detector. Operates
ONLY on pixel data -- an RGB frame array -- and the ball's known static
RENDER color (envs/assets/baseball_park_kc01a.xml's `ball_mat`, rgba
(0.95, 0.18, 0.18, 1), a material/appearance property, not a per-frame
ground-truth position read). No env/mujoco import here at all; this module
cannot leak simulator state even by accident because it has no access to
it.
...
threshold -- correct only for a scene with at most one red object in view
(true here: ball_mat is the only red material in envs/assets/
baseball_park_kc01a.xml). No scipy/skimage dependency; this project does
not otherwise depend on either.
"""
...
def _redness_score(frame_rgb: np.ndarray) -> np.ndarray:
    ...
    was caught empirically: scripts/vm01_vision_baseline.py's first run
    showed 60-170px mean position error at wide FOV, traced to the foul
    line's large white blob dominating the weighted centroid, not the
    actual (sub-pixel) ball. "Excess red" correctly scores true white/gray
...
class BallDetector:
    ...
    def detect(self, frame_rgb: np.ndarray) -> Detection:
        ...
        # 1-pixel "blob" (the sub-pixel regime docs/design/VISION-01.md

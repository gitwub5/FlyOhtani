...
왜 나눴나: 투구 상수가 `world/batter.py`(장면 만들기) 안에 섞여 있었고, 구종을
...
두 모듈로 나뉘어 있고, **그 순서가 중요하다**:
    ...
    pitch   투구의 수치와 기하. `units` 말고는 아무것도 임포트하지 않는다
            (leaf). 그래서 `world.batter`가 이걸 임포트해도 순환이 없다.
    body    마운드 위 투수 파리의 MJCF. `world.batter`의 몸 만들기를 쓰므로
...
from flyohtani.flyshohei.pitch import ARSENAL, STANDARD, Pitch, geometry, get

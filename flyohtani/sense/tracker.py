"""Frame-to-frame ball tracking, producing the policy-facing
VisionObservation stream. No simulator-state import -- this module only
ever sees ball_detector.py's Detection objects (pixel-space) and
proprioception/contact values the CALLER already read from the env's
allowed channels (docs/records/PRIOR-FINDINGS.md section 6); it never
reads ball_pos/ball_vel/remaining itself.

The clock/age/latency contract:

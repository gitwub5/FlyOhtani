...
조사: 2026-09-15~16, 한국 시간. 사용자가 제시한 Stonkfly, Doom/Smash Bros., Minecraft, NeuroMechFly 기반 시뮬레이션, FLM을 원본 프로젝트 설명과 대조했다. 외부 코드를 실행하거나 데이터를 내려받지는 않았다.
...
두 논문의 구체적인 방정식·자극 프로토콜·매개변수 재현 검토는 다음 계획 작업이다. 원 논문의 후각 학습 결과를 시각 기반 공 치기로 옮기는 일은 새로운 가설임을 명시한다.

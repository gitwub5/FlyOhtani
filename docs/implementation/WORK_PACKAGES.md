...
## 최신 통합 지시: 자세·색상 → 거리 점수 → 스윙 개선

[I-08a-style + I-07c 통합 규약](../design/BATTING-QUALITY-AND-SWING.md)을 따른다. 외형 단계의 물리 불변성과 스윙 개선 단계의 물리 재검증을 구분한다. 중앙 투구부터 유효 전방 첫 착지 비거리 점수(1m=1점)를 도입하고 실제 스윙/타구를 비교한다. RL 훈련은 범위 밖이다.

## 추가 작업: I-08a-style

사용자 요청으로 **몸은 옆으로, 머리는 투수 쪽으로** 향하는 자세와 NeuroMechFly 원본 부위별 색상을 적용한다. [구현 명세](../design/FLY-BATTING-STANCE-AND-COLOR.md)를 따른다. 양발 접지·양손 그립·동일 배율과 기존 타격 물리를 유지하며, 현 단계는 시각 자세다.

## 2026-09-16 — I-08a 외형 수용 실패: 수정 우선

원본 wheel/STL 동일성은 확인했지만 뒷발이 약0.92m 떠 있고 앞다리만3배 확대됐다. 사용자 자세 기준은 앞다리2개 그립·뒷다리2개 접지·중간다리 접기다. **다음 작업은 I-08a-fix**이며 기존 완료 보고보다 [외형 재검토와 수정 규약](../records/FLY-VISUAL-REVIEW.md)을 우선한다. 과거 기록은 보존한다.

## 중앙 타격 이후의 다음 작업 (최신)

[팔로우스루·연구용 파리 모델 계획](../design/FOLLOWTHROUGH-AND-FLY-MODEL.md)을 따른다. I-07b-followthrough로 반복 토크 반전과 접촉 파라미터 설명을 정리하고, I-08a로 NeuroMechFly 시각 모델을 적용한다. 이후 나머지8코스를 재보정한다. 실제 전신 역학은 별도 I-08b다. 기존 코드의 중앙 타격 결과는 재현됐지만9코스 전체 완료는 아니다.
...
## 현재 우선순위 — 사용자 타격 방향 지적 반영

**I-07b-fix: 역방향 스윙과 접촉-only 목표 수정**을 먼저 진행한다. B2/변화구/RL 훈련은 보류한다. [검토 증거](../records/B1-BATTING-REVIEW.md)와 [타구 규약](../design/ENV-002-BATTED-BALL.md)을 따른다.

1. +x 인필드 방향으로 공을 보내는 준비 자세·스윙 경로를 다시 보정한다.
2. contact에서 종료하지 않고 impulse 전달/배트 분리/타구 착지까지 동일 환경에서 적분한다.
3. 접촉 보상을 없애고 forward-flight 판정·타구 원지표·별도 보상을 구현한다.
4. 중앙 직구로 먼저 역학을 검증한 뒤9코스 oracle/scripted를 다시 비교한다.
5. 실제 시간축을 맞춘 전체 타구 영상을 만든다. 기존 트리거 표·접촉9/9는 새 성공의 근거로 쓰지 않는다.

코드·수치 모델·구동·접촉의 검증 결과는 Claude가 기록한다. Codex 이번 검토는 기존 코드의 임시 진단과 문서 수정이다.

## 이전 작업 순서 (아래 기록보다 위 타격 수정이 우선)

**현재 야구장 다음 작업: I-07b 잔여(구속 변화) 또는 I-07c(공기력·변화구·선구안). 신경 연구: I-02 → I-04 → I-05. 시각화: I-04의 실제 데이터 후 I-06a, I-05 기록 후 I-06b.** 두 경로는 독립이며 신경 조건화에 물리환경이 필요 없다.
...
| I-07a/b/c | 야구장 B0 / 코스·속도 / 변화구·선구안 | I-07a+I-07a-1+I-07b(조준·9코스) Claude 완료(2026-09-16). 남은 I-07b(구속 변화)/I-07c(공기력·변화구·선구안) 미착수 |
...
**I-07b(조준+9코스) 완료 (2026-09-16, Claude):** [ENV-002-B1-courses.md](../design/ENV-002-B1-courses.md)(구현 전 고정) 따라 `envs/baseball_b1_env.py`(`BaseballB1Env`) + `envs/assets/baseball_park_b1.xml`(B0 파일은 안 건드림)에 틸트 힌지(조준 2번째 축, gear=6 별도 보정)와 9개 코스(zone 중심(0.4318,0,1.0), 반폭0.2159m, 반높이0.35m)를 구현했다. **도달성: oracle(정답 라벨) 9/9 코스 100%** — 도달 불가능 코스 없음. **scripted(관측만) 9개 중 5개만 100%**, 나머지 4개는 선형 조준 fit의 근사오차로 실패(코스별 원인 기록, oracle과 분리 보고). zero_torque/held_rest/random/always_swing_fixed_pose는 9개 코스 전부 0%. 코스별 4카메라 영상(`runs/env002-b1-demo/video/`). 테스트 53개(B1 신규15+기존38) 통과. **시행착오 보존**: 연속 PD·2단계 bang-bang·축별 독립 트리거 계산이 모두 실패했고(스윙-틸트 관성 결합 무시) 최종적으로 두 축을 동시 구동한 채 코스별 트리거 시각을 직접 탐색하는 방식으로 성공했다 — 상세는 `docs/records/VALIDATION_LOG.md`. B1의 구속(속도) 변화, B2/B3, RL 훈련은 범위 밖(사용자 지시).

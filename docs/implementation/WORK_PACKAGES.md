...
[VM-01](../tasks/VISUOMOTOR-PIVOT.md) + 후속(P2/B2/V2) 완료: **V1/V2 눈
영상·시간적 추적**(PASS) · **P1 독립 충돌 검증**(FAIL) → **P2 원인
분리**(기하 문제 확인·해결, 재료 자체는 여전히 기준 미달) · **B1 최소
파리 몸체 설계** → **B2 단위 해결·명세 확정**(완료). 전체 보고
[VM01-REPORT.md](../records/VM01-REPORT.md). KC 추가 탐색/gear 재보정은
보류 유지. **재료 반발계수/침투가 여전히 기준 미달이므로 학습 훈련은
시작하지 않는다** — 다음 배정은 새 진단 profile의 solref/solimp 후보
비교 또는 B2가 남긴 절대 질량/gear 캘리브레이션 중 사용자가 명시적으로
지정한다.
...
| VM-01-V2 | 시간적 추적·예측(칼만 필터) 추가 | 완료 — PASS(가림 구간 오차 28% 개선, 정상 구간은 소폭 악화) |
| VM-01-P1 | 독립 충돌 검증(배트·몸통 제어 제거) | 완료 — **FAIL** |
| VM-01-P2 | 충돌 실패 원인 분리 | 완료 — 기하 문제(판 두께) 확인·해결, 재료 자체(반발계수~0.29·침투~0.05m)는 여전히 기준 미달, [원인·수정안](../records/VM01-P2-CONTACT.md) |
| VM-01-B1 | 파리 관절 최소 몸체 | 설계만 완료(NOT-RUN, 구현 미착수) — [설계](../design/VM01-B1-MINIMAL-BODY.md) |
| VM-01-B2 | 단위 불일치 해결·구현 명세 확정 | 완료 — B1의 단위 오류(측정 버그) 정정, mm/g/μN 확정(출처: neuromechfly.org), 절대 질량·gear는 구현 시 캘리브레이션으로 명시 |

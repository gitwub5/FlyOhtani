...
- **V1(눈 영상·공 추적) — PASS(한계 명시)**: `envs/assets/
  ...
  모두 예상대로 동작(가림 시 검출률 0%, 지터에도 검출률 유지).
- **V2(시간적 추적·예측) — PASS**: `vision/kalman_tracker.py`(등속도
  칼만 필터)가 미검출 구간에서도 마지막 속도로 위치를 전진 추정한다
  (기존 `vision/tracker.py`는 위치를 그대로 정지시켰다 — 비교용으로
  보존). 가림 구간(0.35~0.42s) 평균 위치 오차가 정답 대비 42.2px→
  30.3px로 28% 개선됐다(`docs/records/evidence/
  VM01-V2-tracking-comparison.json`) — 정상 구간에서는 오히려 소폭
  악화(0.91→1.21px, 필터 지연으로 추정)됨을 함께 보고한다. 정답 상태는
  이번에도 평가 스크립트에서만 읽는다(AST 검사로 확인).
- **P1(독립 충돌 검증) — FAIL**: 배트·몸통 제어를 완전히 제거한 분리
  ...
  등록 기준을 통과하지 못했다(`docs/records/VM01-P1-CONTACT.md`).
- **P2(원인 분리) — 완료**: `docs/records/VM01-P2-CONTACT.md`. P1의
  음의 반발계수는 **재료가 아니라 기하 문제**였다 — 공(반지름 0.0366m)이
  판(두께 0.02m)보다 굵어 접촉 도중 공 중심이 판을 지나가며 접촉 법선의
  부호가 뒤집히는 것을 직접 관측했다. 공보다 두꺼운 블록이나 진짜 평면을
  쓰면 반발계수가 양수(~0.29)로 dt-수렴한다. 다만 **그 ~0.29와 침투
  ~0.05m 자체는 기하·충돌각(0°/20°/35° 모두 확인)과 무관하게 여전히
  사전 등록 기준(0.3~0.7 / ≤0.0075m) 미달**이다 — 비스듬한 결과가
  괜찮아 보인다고 정면 실패를 면제하지 않았다. **학습 개시는 계속
  보류한다.**
- **B1(파리 관절 최소 몸체) — NOT-RUN(설계만, 지시된 범위)**:
  ...
  사실을 확보했다. 최소안으로 "지지된 몸통 + 앞다리 1개(7DOF) + 도구
  부착"을 제안하고, 공/도구 크기는 정식 야구공이 아니라 현재 앞다리
  도달거리(0.580m)의 10~20%로 줄일 것을 제안했다 — 인간식 직립·고정
  스윙 순서는 요구하지 않는다.
- **B2(단위 해결·명세 확정) — 완료**: B1이 "raw XML 질량 합≈1.0"이라
  보고했던 것은 **측정 오류**였다(정규식이 `<statistic meanmass="1.0">`
  메타데이터까지 잘못 집어 더함) — 오염 제거 후 실제 질량 합은 0.001.
  원본 FlyGym 로더(`flygym/fly.py`)에 질량 재조정 코드가 없음을
  확인하고, NeuroMechFly 공식 문서에서 **길이=mm, 질량=g, 힘=μN** 단위
  규약을 직접 확인했다(출처:
  [neuromechfly.org](https://neuromechfly.org/tutorials/1b_advanced_model_composition/)).
  이 단위로 앞다리 세그먼트 질량 비율과 원본 제어 게인/토크한계를 SI로
  환산해 구현 명세를 확정했다(`docs/design/VM01-B1-MINIMAL-BODY.md` §8)
  — 절대 질량(%)과 실제 gear/토크는 제1원리 스케일링 대신 KC-01a식
  무공 스윕으로 구현 시 캘리브레이션하도록 명시적으로 남겼다.
- **연구 트랙 구분**: 위 여섯 작업은 모두 일반 정책(환경 기준선) 트랙이다.
...
`python -m pytest`/`pytest` 두 진입점 모두 **138 passed**로 일치(B1/B0/
ENV-001 87개 + KC-01a 22개 무변경 + VM-01 vision V1 16개 + P2 6개 + V2
7개 신규). lint 클린(`vision/` 포함). 비editable wheel 설치로 모델
로드까지 확인(`docs/records/evidence/R03-wheel-install-check.txt`) —
`vision` 패키지 추가로 `pyproject.toml`의 `packages` 목록을 갱신하고
재설치했다.

## 다음 작업

[VM-01](../tasks/VISUOMOTOR-PIVOT.md)의 V1/V2/P1/P2/B1/B2를 모두 완료해
보고했다([전체 보고](VM01-REPORT.md)) — V1/V2 PASS, P1 **FAIL**→P2로
원인 분리(재료 자체는 여전히 기준 미달), B1 설계+B2 단위 해결·명세
확정. **재료 반발계수/침투가 여전히 기준 미달이므로 학습 개시는 계속
보류한다.** 다음은 사용자가 명시적으로 지정한다: (1) 새 진단 profile에서
solref/solimp 후보 비교(`docs/records/VM01-P2-CONTACT.md`), (2) B2가
남긴 미확정 항목(절대 질량 %, 관절 하드 리밋, gear/토크 캘리브레이션)
해결 후 실제 구현, (3) KC 후보 탐색/gear 재보정은 이 전환으로 계속
보류. 기존 실패/철회 기록은 그대로 보존한다.

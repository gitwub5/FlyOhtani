...
- **지면-몸통 접촉의 숨은 전제**: B1의 `batter_body`는 관절이 없어(용접) MuJoCo가
  ...
  영향 없다.
- **torso_only는 유효 타구를 만들지 못했다**: 최선의 트리거 시각(0.360s)에서도
  접촉점 속도 최대 ≈4.86 m/s로, swing 단독(≈7 m/s대)에 못 미친다. gear를 swing과
  동일하게 맞춰도 몸통(35kg)의 관성이 배트(0.9kg)보다 훨씬 커서 유효 타격을
  만들기엔 부족했다 — **몸통 추가가 개선을 보장하지 않는다는 사실을 그대로
  보고한다**(비목표를 가정하지 않음). 상세 수치는 `docs/records/KC-01a-COMPARISON.md`.
- **"고정" 축을 어떻게 구현하는지가 결과를 바꾼다**: `arm_only`/`torso_only`의
  비참여 축을 `env.set_held_pose()`(매 substep qpos/qvel을 강제로 0 재설정)로
  하드 고정하면, 동일한 swing 트리거 시각(0.09425316355759385s)을 0.03~0.30s
  전 구간에서 다시 스윕해도 **단 하나도 유효 타구가 나오지 않았다**. 컨트롤러
  자체의 유지 게인(P-hold, gain=50)으로만 "고정"하면 같은 트리거 시각에서 정상
  타구가 나온다(§6 표). 두 방식 모두 목표 각도는 정확히 0으로 유지하지만, 하드
  고정은 매 substep 속도를 강제로 지워 접촉 순간의 미세한 반작용 흡수 방식을
  바꾼다 — 이미 알려진 극도의 타이밍 민감도(B1의 ±5ms 결과와 동일 성격)를 감안하면
  그럴듯한 설명이다. **최종 비교(§6)는 컨트롤러 유지 방식을 사용한다** — "고정"은
  "용접"이 아니라 "0 회전을 능동적으로 유지"로 정의한다.

## 6. 비교 조건 (구현 후 실행)

| 조건 | torso | swing | tilt |
| --- | --- | --- | --- |
| `arm_only` | 유지(0) | 트리거 0.09425316355759385s → -1.298 | 유지(0) |
| `torso_only` | 트리거 0.360s → +0.496 | 유지(-1.96) | 유지(0) |
| `simultaneous` | 트리거 0.096s → -0.4 | 트리거 0.096s(동시) → -0.726 | 유지(0) |
| `staggered` | 트리거 0.242s → -0.4 (swing보다 0.12s 먼저) | 트리거 0.122s → -0.726 | 유지(0) |

"유지"는 컨트롤러 자체의 P-hold(gain=50)로 목표각을 능동 유지하는 것이며, 환경
레벨 qpos/qvel 강제 고정이 아니다(§5).

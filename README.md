...
**눈을 못 쓰는 최고 정책을 접촉률로는 시드 3개 모두, 보상으로는 3개 중 2개**
이겼다(아래 [결과](#결과)).
...
프레임·정해진 자세로 휘두르는 가장 좋은 정책을 **접촉률로는 시드 3개 모두,
보상으로는 3개 중 2개** 이겼다.

| | 평가 보상 | 접촉 | 페어 | 존 판독 |
| --- | --- | --- | --- | --- |
| **학습한 회로** (시드 3·4·5) | **1.537 · 0.980 · 0.696** | **0.70 · 0.43 · 0.50** | 0.43 · 0.27 · 0.13 | 0.67 · 0.67 · 0.47 |
| 눈을 못 쓰는 최고 정책 | 0.751 | 0.37 | 0.17 | — |

존 판독 **0.67은 구조적 상한이다** — 평가 세트의 1/3이 가운데 공인데 정책은
높음/낮음 두 가지만 출력한다. 시드 3·4는 높은 공 10/10, 낮은 공 10/10으로
정확히 그 상한에 닿았다.

정말 눈을 쓰는지는 따로 확인했다 — **투구를 2 ms 늦추면 트리거도 정확히
1프레임(2.08 ms) 늦어진다.** 시계를 세고 있었다면 그대로였을 것이다.
맨 위 영상은 이전 실행(D36) 정책의 타구로, 808 mm/s · 발사각 +10° ·
비거리 28.9 mm(사람 환산 약 14 m)다.

> ⚠️ **2026-09-24 현재 위 숫자는 낡았다.** D37(스윙 40 → 19 ms)과 D38(왼쪽 눈
> 20° → 15°)로 몸과 눈이 바뀌었고, 그 결과 파리가 결정할 때 도착해 있는 루밍
> 신호가 **44% → 100%**가 됐다. 위 성적은 전부 **바뀌기 전** 설정에서 잰 것이고,
> 새 설정의 성적은 아직 없다. 재실행이 다음 작업이다
> ([STATUS](docs/records/STATUS.md)).
>
> 📝 **2026-09-23에 위 숫자가 크게 바뀌었다.** 존을 읽는 파라미터가 탐색 범위
> 밖에 있어 **죽어 있었고**(모든 투구를 "높은 공"으로 읽었다), 동시에 기준선이
> 가운데 자세에 묶여 있어 **너무 약했다.** 둘 다 고치자 접촉률은 0.30 → 0.70,
> 기준선은 0.471 → 0.751로 올랐다 — 즉 **양쪽이 같이 강해졌고 상대적 우위는
> 거의 그대로다**(1.45배 → 1.43배). 경위는
> [LEARNING-01](docs/records/LEARNING-01.md) 3차.

> ⚠️ **배선을 무작위로 섞은 대조군이 이 성적을 이긴다.** 대조군에도 학습
> 전체를 돌려 같은 투구 30개로 쟀더니, 3쌍 중 1쌍에서 셔플이 앞섰고(0.696 대
> **1.244**) 그 셔플은 **맹목 기준선도 넘었으며 존도 읽었다.** 그러니 위 표를
> **연결망 구조 덕이라고 말할 수 없고**, "눈 못 쓰는 정책을 이겼다"는 것도
> 연결망의 증거가 아니다. 판정 기준은 실행 전에 고정했고, 원인 진단과 다음
> 단계는 [BRAIN-CIRCUIT](docs/records/BRAIN-CIRCUIT.md)에 있다.

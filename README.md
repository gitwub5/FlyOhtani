...
**눈을 못 쓰는 최고 정책을 시드 3개 모두 이겼다**(아래 [결과](#결과-한눈에)).
...
보는 **왼쪽 눈은 시야 30°의 예민한 영역**(0.94°/픽셀)으로 투구선을 겨누고,
...
| **VM-01** 시각 | 축척 그대로의 공은 **안 보인다**(각지름 0.25°, 픽셀은 3.75°). 180·576·256조합을 재서 공 2배 + 조준한 30° 눈으로 결론. 눈 프레임률은 병목이 아니었다 |
| **타석 충돌 확인** | K1~K6 전부 통과. 반발계수 0.41~0.46, 에너지 생성 없음 |
| **VM-01** 투구 | 느린 공은 중력이 띄운다. 결정 시점 계산을 정정한 뒤에는 **실제 투수판(34.2 mm)에서 52 ms**에 던져도 파리가 보고 휘두를 수 있다 |
| **스윙 물리** | 기존 스윙은 접촉 때 **−18°로 내려가며** 공을 눌렀다(공은 −23°로 하강). 준비 자세를 다시 찾아 **+7~+11.5°로 밀어치게** 하자 비거리 0.3 → 66 mm |
| **뇌 회로** | 망막 → LC4·LPLC2 → 하강뉴런까지 스파이크가 흐른다. **투구가 2 ms 늦으면 트리거도 정확히 1프레임 늦는다** — 시계가 아니라 눈을 쓴다는 뜻 |
| **학습** | 처음 보는 투구 30개에서 **시드 3개 모두 맹목 기준선 초과**(보상 0.877 / 0.566 / 0.613 vs 0.471). 접촉률 0.23~0.30 |
| **셔플 대조군** | 배선을 섞어도 같은 성적이 나온다 — **연결망 구조가 성능의 원인이라는 증거는 아직 없다**([BRAIN-CIRCUIT](docs/records/BRAIN-CIRCUIT.md)) |
| **녹화한 타구** | 학습된 정책: 808 mm/s, 발사각 +10°, 비거리 28.9 mm(사람 환산 14 m) 페어 |
...
- 공은 축척보다 **2배 크고**, 왼쪽 눈은 실제 파리보다 **6배 예민**하며, 마운드는
  실제 환산 **60 m**에 있다. 그래야 파리가 공을 보고 휘두를 수 있기 때문이고,
  무엇을 포기했는지는 [VM-01](docs/records/VM-01-EYE-RATE.md)에 적혀 있다.

...
<sub>녹화한 안타 한 장면(실제보다 133배 느리게). 커쇼급 직구를 받아쳐 좌중간으로 보낸 라인드라이브 — 사람 크기로 환산하면 약 15 m. <b>스윙은 아직 스크립트다</b> — 학습된 동작이 아니다.</sub>
...
<sub>38 ms 데모 스윙. 관절 최고 속도 287 rad/s로 생물학적 상한(300 rad/s) 안이고, 이 몸이 접촉 순간에 낼 수 있는 최대 배트 속도(390 mm/s)를 낸다.</sub>
...
| **VM-01** 투구 | 느린 공은 중력이 띄운다(발사각 +21°). **커쇼급 직구**(93 mph의 Froude 환산 1888 mm/s)를 122 mm에서 던져야 평평해진다(+5.6°) |
| **녹화한 안타** | 타구 740 mm/s(오타니 환산의 32%), 발사각 +14°, 비거리 29.9 mm — **사람 크기로 약 15 m 라인드라이브** |
| **녹화한 헛스윙** | 타이밍이 3 ms만 어긋나도 헛친다(접촉 창 약 ±1 ms) |
